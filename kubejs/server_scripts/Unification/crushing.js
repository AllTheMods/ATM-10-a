let CRUSHING_COMPAT_TARGETS = [
  {
    ...
    excessiveUtilitiesCrusher: true,
...
ServerEvents.recipes((allthemods) => {
  function addCrushingCompatRecipes(target) {
    let id = `${target.toId.split(':').pop()}_from_${('tag' in target.from ? target.from.tag.split(/[\/:]/).pop() : target.from.item.split(':').pop())}`;
    ...
    if (target.excessiveUtilitiesCrusher) {
      allthemods.custom({
      ...
      }).id(`allthemods:compat/excessive_utilities/crusher/${id}`);
    }
